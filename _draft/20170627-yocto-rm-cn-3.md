...
[`do_patch`][3788] task 通过变量 [`SRC_URI`][3789] 找到可用的补丁文件，然后处理 recipe，补丁文件默认都是 `*.patch` 或者 `*.diff` 文件，或者任何在 `SRC_URI` 中指定了 "apply=yes" 的文件。
...
*	_[`do_prepare_recipe_sysroot`][29]:_  这个 task 在 `${`[`WORKDIR`][831]`}` 设置了两个 sysroots （比如，`recipe-sysroot` 和 `recipe-sysroot-native`），这样一来 sysroots 就包含了 recipe 的 [`do_populate_sysroot`][832] task 的内容，而这个 recipe 包含了这个 task 的依赖。一个 sysroot target 和 运行在主机系统的 native 二进制文件中。
...
### 3.5.7. Stamp Files and the Rerunning of Tasks[¶][445]
